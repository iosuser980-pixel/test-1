...
if not main_frame then
    ...
    main_frame.Size = UDim2.new(0, 420, 0, 300)
    ...
    logs_tab_btn.MouseButton1Click:Connect(function()
        ...
        refresh_btn:CaptureFocus() -- minor hack to ensure refresh is visible
        refresh_btn:ReleaseFocus()
...
_G.BackToLobby = _G.BackToLobby or true

-- wiring toggles to existing functions
local toggles = {}

toggles.autoskip = make_toggle(controls_frame, 0, "Auto Skip (vote skip)", _G.AutoSkip, function(state)
    _G.AutoSkip = state
    if state then
...
toggles.autosnow = make_toggle(controls_frame, 36, "Auto Snowballs (collect)", _G.AutoSnowballs, function(state)
...
toggles.antilag = make_toggle(controls_frame, 72, "Anti-Lag (destroy visuals)", _G.AntiLag, function(state)
    _G.AntiLag = state
    if state then
...
toggles.backtolobby = make_toggle(controls_frame, 108, "Auto Post-Match -> Lobby", _G.BackToLobby, function(state)
    _G.BackToLobby = state
    if state then
        Log("Enabling BackToLobby handling")
        start_back_to_lobby()
    else
        Log("Disabling BackToLobby handling")
    end
end)

toggles.sendwebhook = make_toggle(controls_frame, 144, "Send Webhook (post match)", _G.SendWebhook, function(state)
    _G.SendWebhook = state
    Log("SendWebhook set to", state)
end)

-- optional quick actions
local actions_label = Instance.new("TextLabel")
actions_label.Size = UDim2.new(1, -20, 0, 20)
actions_label.Position = UDim2.new(0, 10, 0, 188)
...
local function make_action_button(parent, x, text, callback)
    ...
    b.Position = UDim2.new(0, x, 0, 210)
    b.Text = text
...
make_action_button(controls_frame, 0.33, "Ready/Start", function() lobby_ready_up(); Log("Attempted to ready/start") end)
make_action_button(controls_frame, 0.66, "Teleport Lobby", function() send_to_lobby(); Log("Requested teleport to lobby") end)
...
local function handle_post_match()
    ...
    if not ui_root then 
        if _G.BackToLobby then
            send_to_lobby() 
            Log("No rewards UI found; teleporting to lobby as fallback")
        end
        return 
    end
    ...
    if not _G.SendWebhook then
        if _G.BackToLobby then
            send_to_lobby()
            Log("SendWebhook disabled; returning to lobby")
        end
        return
    end
...
if screen_gui and screen_gui.MainFrame then
    ...
    if f.ControlsFrame then
        ...
        for k, setter in pairs(toggles) do
            ...
            if k == "backtolobby" then setter(_G.BackToLobby) end
            if k == "sendwebhook" then setter(_G.SendWebhook) end
        end
    end
end
